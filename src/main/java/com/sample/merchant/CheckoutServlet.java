	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			if (request.getRequestURI().contains("SetExpressCheckout")) {
				...
				String amountItems = String.valueOf(new BigDecimal(Double.parseDouble(request.getParameter("itemAmount"))).setScale(2, RoundingMode.UP).doubleValue());
				/*
				...
				if (request.getParameter("shippingTotal") != "") {
					...
					shippingTotal.setCurrencyID(CurrencyCodeType
							.fromValue(request.getParameter("currencyCode")));
					orderTotal += new BigDecimal(Double.parseDouble(request.getParameter("shippingTotal"))).setScale(2, RoundingMode.UP).doubleValue();
				...
				if (request.getParameter("insuranceTotal") != "") {
					...
					orderTotal += new BigDecimal(Double.parseDouble(request.getParameter("insuranceTotal"))).setScale(2, RoundingMode.UP).doubleValue();
				}
				/*
				...
				itemsTotal.setValue(Double.toString(new BigDecimal(itemTotal).setScale(2, RoundingMode.UP).doubleValue()));
				//PayPal uses 3-character ISO-4217 codes for specifying currencies in fields and variables. 
				...
				paydtl.setOrderTotal(new BasicAmountType(CurrencyCodeType.fromValue(request.getParameter("currencyCode")),
						String.valueOf(new BigDecimal(orderTotal).setScale(2, RoundingMode.UP).doubleValue())));
